...
main()
{
    ...
    sprintf(fn_req, "additionalDecapTesting_%d.req", CRYPTO_SECRETKEYBYTES);
    if ( (fp_req = fopen(fn_req, "w")) == NULL ) {
        printf("Couldn't open <%s> for write\n", fn_req);
        return KAT_FILE_OPEN_ERROR;
    }
    sprintf(fn_rsp, "additionalDecapTesting_%d.rsp", CRYPTO_SECRETKEYBYTES);
    ...
    do {
        ...
        if ( (ret_val = crypto_kem_dec(ss, ctUnderTest[count], skUnderTest[count])) != 0) {
